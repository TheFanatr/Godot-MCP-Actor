...
func send_notification(notification: Dictionary) -> void:
	for client_id in clients.keys():
		var client = clients[client_id]
		if client.ws.get_ready_state() == WebSocketPeer.STATE_OPEN:
			var json_text = JSON.stringify(notification)
			var result = client.ws.send_text(json_text)
			if result != OK:
				print("Error sending notification to client %d: %d" % [client_id, result])
